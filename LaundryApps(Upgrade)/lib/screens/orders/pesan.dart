...
class _PesanPageState extends State<PesanPage> {
  ...
  Future<void> _generateQrisBarcode() async {
    ...
    try {
      ...
      // Automatically display large high-impact QRIS modal
      if (mounted) {
  ...
  Future<void> _saveOrderAsDeferredQris() async {
    ...
    if (totalItems <= 0 || total <= 0) return;
    ...
    try {
      ...
      for (var it in cartItemsList) {
        ...
        if (_claimFreeWash && isWash && !freeApplied) {
          ...
          final remainingQty = qty - 1;
          if (remainingQty > 0) {
            final remainingWeights = itemTubNotes.length > 1 ? itemTubNotes.sublist(1) : <String>[];
            final normalNote = TubNoteHelper.formatTubNotes(
              weights: remainingWeights,
              startIndex: 2,
              generalNote: _notes[it.id ?? 0],
              prefix: 'Cuci',
          ...
          final prefix = isWash ? 'Cuci' : (isDry ? 'Kering' : 'Item');
          final String itemNote = isMachine
              ? TubNoteHelper.formatTubNotes(
                  weights: itemTubNotes,
                  startIndex: 1,
                  generalNote: _notes[it.id ?? 0],
      ...
      Globals.showSuccessSnackBar(
        'Transaksi #${finalOrder.id} (${finalOrder.customerName}) disimpan sebagai QRIS Tertunda. Sistem akan otomatis memverifikasi status pembayaran di latar belakang.',
      );

      _clearCart();
    } catch (e) {
      Globals.showWarningSnackBar('Gagal menyimpan transaksi: $e');
    } finally {
      if (mounted) setState(() => _isSubmitting = false);
    }
  }
                      ...
                      children: [
                        ...
                        Expanded(
                          child: OutlinedButton.icon(
                            ...
                            icon: const Icon(Icons.refresh_rounded, size: 16, color: StyleConstants.primaryColor),
                            label: const Text(
                              'Cek Status',
                              style: TextStyle(fontWeight: FontWeight.w800, fontSize: 12.5, color: StyleConstants.primaryColor),
                            ),
                            style: OutlinedButton.styleFrom(
                              side: const BorderSide(color: StyleConstants.primaryColor),
                              padding: const EdgeInsets.symmetric(vertical: 13),
                              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                            ),
                          ),
