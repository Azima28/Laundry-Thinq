...
class _MesinPengeringScreenState extends State<MesinPengeringScreen> {
  ...
  Future<void> _loadBaseUrl() async {
    ...
    String url = prefs.getString('machines_base_url_pengering') ?? 'http://127.0.0.1:5001/';
    if (url.contains('azima.local')) {
      url = url.replaceAll('azima.local', '127.0.0.1');
      await prefs.setString('machines_base_url_pengering', url);
    }
  ...
  Future<void> _loadCustomOrder() async {
    final prefs = await SharedPreferences.getInstance();
    final list = prefs.getStringList('machines_order_pengering') ?? [];
    setState(() => _customOrder = list);
  }

  Future<void> _saveCustomOrder() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setStringList('machines_order_pengering', _customOrder);
  }
  ...
  Future<void> _saveEntity(String name) async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('✅ Pengering "$name" berhasil disimpan ke Database!'), backgroundColor: Colors.green),
      );
    }
    _loadSavedMachines();
  }
  ...
  Future<void> _removeSavedEntity(String name) async {
    ...
    if (machine.id != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('🗑️ Pengering "$name" berhasil dihapus dari Database!'), backgroundColor: Colors.redAccent),
        );
      }
      _loadSavedMachines();
    }
  ...
  Future<void> _fetchFromUrl() async {
    ...
    try {
      final response = await http.get(Uri.parse('${base}api/dryer/discover')).timeout(const Duration(seconds: 10));
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('✅ Berhasil mendeteksi pengering ThinQ!'), backgroundColor: Colors.green),
          );
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _testUrl(String entityId) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('${base}api/dryer/state'),
  ...
  Future<void> _showEditDialog(String entityId, String currentTitle) async {
    ...
    await showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Edit Nama Tampilan Pengering'),
        content: TextField(
          controller: ctrl,
          decoration: const InputDecoration(labelText: 'Nama Custom Pengering', border: OutlineInputBorder()),
        ),
        actions: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Pengaturan Mesin Pengering', style: TextStyle(fontWeight: FontWeight.bold)),
                  ...
                  Container(
                    ...
                    child: const Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Tarik dan lepas (drag-and-drop) card mesin di sisi kanan untuk mengurutkan tata letak tampilan mesin pengering kasir.',
                                ...
                                return Card(
                                  ...
                                  child: ListTile(
                                    ...
                                    leading: CircleAvatar(
                                      ...
                                      child: Icon(Icons.dry_rounded, color: saved ? Colors.green : Colors.grey),
                                    ),
