...
class _HistoryGosokPageState extends State<HistoryGosokPage> {
  ...
  Future<void> _loadOrders() async {
    ...
    // Filter for ironing orders only
    final ironOrders = all.where((order) => 
      order.items.any((item) => ironItemIds.contains(item.itemId))
    ).toList();

    // Filter by selected date on date string (YYYY-MM-DD)
    final dateStr = _formatDateString(_selectedDate);
    final dayOrders = ironOrders.where((o) => o.orderDate.toIso8601String().startsWith(dateStr)).toList();
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (context, setStateDialog) => AlertDialog(
          ...
          title: const Text('Pelunasan Pembayaran Gosok', style: TextStyle(fontWeight: FontWeight.bold)),
          content: Column(
        ...
        content: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text('Layanan Gosok (Setrika):', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Color(0xFF475569))),
              ...
              ...order.items.map((it) => Padding(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Expanded(child: Text(it.itemName, style: const TextStyle(fontSize: 13))),
                        Text(formatRp(it.price), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                      ],
                    ),
                  )),
              const SizedBox(height: 16),
          ...
          if (order.status.toLowerCase() != 'selesai')
            ElevatedButton.icon(
              onPressed: () async {
                ...
                final updated = order.copyWith(status: 'Selesai');
                await _databaseHelper.updateOrder(updated);
                
                // 2. Load orders to refresh UI
                _loadOrders();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Riwayat Gosok (Setrika)', style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text('Rangkuman Gosok', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Color(0xFF0F172A))),
