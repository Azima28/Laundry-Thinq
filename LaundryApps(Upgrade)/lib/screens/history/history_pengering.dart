...
class _HistoryPengeringPageState extends State<HistoryPengeringPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      // Fetch machine usage history for dryers only using the new database filter
      final usageRecords = await _db.getMachineUsageHistory(type: 'pengering');
      List<Map<String, dynamic>> dryerUsage = usageRecords;

      // Apply date range filter
      List<Map<String, dynamic>> dateFiltered = dryerUsage;
      dateFiltered = dryerUsage.where((record) {
  ...
  void _showStatistics() {
    int totalUsage = _filteredRecords.length;
    int countSuccess = 0;
    int countFailed = 0;
    int countSended = 0;
          ...
          child: Column(children: [
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                Text('Statistik Pengering', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                const Spacer(),
            ...
            Expanded(
              child: ListView(controller: scrollController, padding: const EdgeInsets.all(20), children: [
                ...
                // Activation Status
                _pSectionTitle('STATUS AKTIVASI'),
                const SizedBox(height: 12),
                Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.grey.shade200)),
                  child: Column(children: [
                    _pStatusBar(Icons.check_circle, 'Berhasil', countSuccess, Colors.green, totalUsage),
                    const SizedBox(height: 12),
                    _pStatusBar(Icons.send, 'Terkirim', countSended, Colors.blue, totalUsage),
                    const SizedBox(height: 12),
                    _pStatusBar(Icons.cancel, 'Gagal', countFailed, Colors.red, totalUsage),
                  ]),
                ),
                const SizedBox(height: 24),

                // Per machine breakdown
                _pSectionTitle('Detail per Mesin'),
                const SizedBox(height: 12),
                    ...
                    return Container(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                          Row(children: [
                            Icon(Icons.wb_sunny_rounded, size: 16, color: Colors.orange),
                        ...
                        ClipRRect(borderRadius: BorderRadius.circular(4), child: LinearProgressIndicator(value: pct, backgroundColor: Colors.grey[200], valueColor: const AlwaysStoppedAnimation(Colors.orange), minHeight: 6)),
  ...
  Widget _pStatCard(String label, String value, Color color, IconData icon) {
    return Container(
      padding: const EdgeInsets.all(14),
  ...
  Widget _pSectionTitle(String t) => Container(
    padding: const EdgeInsets.symmetric(vertical: 8, horizontal: 12),
    decoration: BoxDecoration(color: Colors.orange.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(8)),
    child: Text(t, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.orange, letterSpacing: 1.2)),
  );

  Widget _pStatusBar(IconData icon, String label, int count, Color color, int total) {
  ...
  void _applySearch() {
    ...
    setState(() {
      if (q.isEmpty) {
        _filteredRecords = _usageHistory;
      } else {
        _filteredRecords = _usageHistory.where((record) {
  ...
  Future<void> _retryActivation(Map<String, dynamic> record) async {
    ...
    try {
      final String endpoint = machine.url;
      final String sanitizedName = machine.name
          .replaceAll(RegExp(r'[^a-zA-Z0-9]'), '_')
          .toLowerCase();
      final String fullUrl = endpoint.endsWith('/')
          ? '${endpoint}on/$sanitizedName'
          : '$endpoint/on/$sanitizedName';
  ...
  Widget _buildHistoryCard(Map<String, dynamic> record) {
    ...
    final machineNameDisplay =
        record['machine_name'] as String? ?? 'Mesin Tak Dikenal';
    final customerName =
        record['customer_name'] as String? ?? 'Pelanggan Tak Dikenal';
